...
+ I programming, the name of a function (or object, type, template, etc) is called its identifier.
...
+ Data is any information that can be moved, processed, or stored by a computer. A single piece of data is called a value. Common examples of values include letters, numbers, and text.
+ A variable is a named piece of memory that we can use to store values. In order to create a vvariable, we use a statement called a definition statement. When the program is run, each defined variable is instantiated, which means it is assigned a memory address.
...
+ std::endl outputs a newline character, forcing the console cursor to move to the next line, and flushes any pending output to the console. The '\n' character also outputs a newline character, but lets the system decide when to flush the output. Be careful not to use '/n'.
+ std::cin and operator>> allow us to get a value from the keyboard.
+ A variable that has not been given a value is called an uninitialized variable. Trying to get the variable will result in undefined behavior, which can manifest in any number of ways.
...
------------------------------------------
QUIZ TIME
+ Question 1: What is the different between initialization and assignment? How many times can a variable be initialized or assigned a value?
