...
+ Consider the following series of statements, each of which defines a variable and initializes it:
    ...
    int main()
    {
        ...
        int d{c};          // initialize variable d with variable value 10
        int e{five()}      // initialize variable e with function return value 5
    }
+ Note that the initializers above make use of a variety of different entities: literals, variables, operators, and function calls. Somehow, C++ is converting all of these different things into a single value that can then be used as the inital value for the variable.
...
+ Wherever a single value is expected in C++, you can use an value-producing expression instead, and the expression will be evaluated will be evaluated to produce a single value.
...
+ Certain expression (such as x = 5) are used primarily for their side effects (in this case, to assign the value 5 to the variable x) rather than the value they produce.
...
+ Consider the following expressions:
    ...
    → Simplifying a bit, a subexpression is an expression used as an operand. For example, the subexpression of x = 4 + 5 are x and 4 + 5. The subexpressions of 4 + 5 are 4 and 5.
    → A full expression is an expression that is not a subexpression. All three expressions above are full expressions.
